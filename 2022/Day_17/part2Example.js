...
const getNextDirection = () => {
	let jetIndex = directionIndex % jets.length;
	let direction = jets[jetIndex];
	directionIndex++;
	return [direction, jetIndex];
};

const summarize = () => {
	let oldHeight = Array(7).fill(-20);

	for (let pair of solid) {
		let [x, y] = pair.split(',').map(Number);
		oldHeight[x] = Math.max(oldHeight[x], y);
	}

	let top = Math.max(...oldHeight);
	return oldHeight
		.map((v) => {
			return top - v;
		})
...
while (rockCount < totalRockCount) {
	...
	if (hasIntersectionSolid) {
		...
		let key = [jet[1], rockIndex, summarize()].join(',');
		if (seen[key]) {
			let [oldRockCount, oldHeight] = seen[key];
			let remainder = totalRockCount - rockCount;
			let repetitions = Math.floor(remainder / (rockCount - oldRockCount));
			offset = repetitions * (height - oldHeight);
			rockCount += repetitions * (rockCount - oldRockCount);
			seen = {};
		}
