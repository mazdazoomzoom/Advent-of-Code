...
const main = async () => {
	try {
		...
		let usedSpace = fileDirectoryStructure.size;
		let freeSpace = 70000000 - usedSpace;
		let spaceToFreeUp = spaceNeeded - freeSpace;

		// Now that we have the size of each directory
		// We need to size of a directory that is the smallest in size that is greater than the space to free up
		let smallestDirectorySize = 0;
		let smallestDirectory = null;
		let findSmallestDirectory = (directory) => {
			if (directory.size > spaceToFreeUp) {
				if (smallestDirectorySize === 0) {
					smallestDirectorySize = directory.size;
					smallestDirectory = directory;
				} else if (directory.size < smallestDirectorySize) {
					smallestDirectorySize = directory.size;
					smallestDirectory = directory;
				}
			}
			for (let i = 0; i < directory.directories.length; i++) {
				findSmallestDirectory(directory.directories[i]);
			}
		};
		findSmallestDirectory(fileDirectoryStructure);

		// Log the smallest directory and its size
		console.log(smallestDirectory.name, smallestDirectory.size);
	} catch (error) {
		console.error(error);
	}
};
