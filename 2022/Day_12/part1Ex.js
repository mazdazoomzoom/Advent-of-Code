...
const lines = readFileSync('inputSample.txt', { encoding: 'utf-8' }) // read day??.txt content
...
function dijkstra(map, start, end) {
	...
	while (queue.length) {
		...
		console.log(`Before filter ${queue.length}`);
		queue = queue.filter((x) => x !== u);
		console.log(`After filter ${queue.length}`);
